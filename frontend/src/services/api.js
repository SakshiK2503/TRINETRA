...
const demoApi = {
  ...
  async getSafeRoute(payload) {
    await sleep(1100);
    const source = payload?.source || "Current location";
    ...
    return {
      ...
      safestPath: [
        "Main Rd (well-lit)",
        "Market Circle (high footfall)",
        "Police Chowki Junction",
        destination,
      ],
      avoidZones: ["Nehru Industrial Lane", "Canal Backroad"],
      lightingScore: 8.1,
      crowdScore: 7.4,
      incidentDensityScore: 2.6,
      liveShareUrl: "https://trinetra.app/live/demo-share-id",
...
const service = {
  ...
  async analyzeHarassment(text, file) {
    try {
      ...
      return demoApi.analyzeHarassment({ text, file });
    }
  },
  ...
  async getSafeRoute(source, destination, options = {}) {
    try {
      ...
      return demoApi.getSafeRoute({ source, destination, ...options });
    }
  },
};
